    ...
    <div class="card mb-1" style="border-radius: 1.5rem;">
        ...
        :style="cardBodyStyles">
            ...
            <div class="flex-grow-1 d-flex  align-items-center" @click="toggleSeparator" style="">
                ...
                <span class="" style="margin-right: 1.5rem;">{{ dataUser.id }}</span>
            </div>
            ...
            <div class="123123">
                ...
                <button class="btn btn-icon mx-1 px-2 d-inline text-dark" @click="this.$router.push({ path: '/models', query: { 'user': dataUser.id } });">
                    <i class="bi bi-layers" style="font-size: 18px;"></i>
                </button>

                <button data-bs-toggle="modal" :data-bs-target="'#editModal_' + dataUser.id" class="btn btn-icon d-inline text-primary px-2"><i class="bi bi-pen"></i></button>  
                
                <div class="modal fade" :id="'editModal_' + dataUser.id" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                ...
                                <input type="text" class="form-control" placeholder="name@example.com" v-model="updatedUser.name">
                                <label for="floatingInput">Имя</label>
                                </div>
                                <div class="form-floating mb-3">
                                <input type="text" class="form-control" placeholder="Password" v-model="updatedUser.slug">
                                <label for="floatingPassword">Слаг</label>
                                </div>
                            </div>
                            <div class="modal-footer ">
                                <button @click="updateUser()" type="button" class="btn btn-second w-100" data-bs-dismiss="modal">Изменить</button>
                            </div>
                        </div>
                    </div>
                </div>

                <button @click="deleteUser()" class="btn btn-icon d-inline text-danger px-2"><i class="bi bi-trash3"></i></button>
            </div>
        </div>
        <div class="separator-card" v-show="isCollapsed"></div>
        <div :id="'collapseProduct' + dataUser.id" class="collapse" aria-labelledby="headingExampleTwo" data-bs-parent="#collapseIndicatorExampleOne" >
            <div class="card-body">
                ...
                <div v-if="!orderLoading">
                    <div v-if="dataOrders.models.length != 0">
                        <div v-for="model in dataOrders.models" :key="user.id" class="card mb-1" style="border-radius: 1.5rem;">
                            <div class="card-body elementSecondList py-1 px-3 d-flex align-items-center" id="headingExampleTwo" aria-controls="collapseIndicatorChevron">
                                ...
                                <button @click="deleteBiguser()" class="btn btn-icon d-inline text-danger px-2"><i class="bi bi-trash3"></i></button>
                            </div>
                        </div>
                    </div>
...
export default {
    ...
    methods: {
        ...
        async updateUser() {
            ...
            try {
                const response = await axios.put(`/user/${this.dataUser.id}`, formData);
                if (!response.status == 200) {
                    this.showErrorToast(response.status, response.data)
                    console.log(response);
                }
        ...
        async deleteUser() {
            this.setLoading(true);
            try {
                const response = await axios.delete(`/user/${this.dataUser.id}`);
                if (!response.status == 200) {
                    this.showErrorToast(response.status, response.data)
                    console.log(response);
                }
        ...
        async toggleSeparator(event) {
            ...
            const collapseTarget = document.getElementById('collapseProduct' + this.dataUser.id);
            ...
            if (this.isCollapsed) {
              await this.getOrders(this.dataUser.id);
            }          
        },
    }
}
