<template>
    <div class="container-fluid text-center" style="height: 100%; display: flex; flex-direction: column;">
        <div class="row mb-3">
            <div class="d-flex align-items-center flex-grow-1" >
                <div class="input-group align-items-center">
                    <span @click="searchColor" class="input-group-text" id="basic-addon1" style="border-radius: 1.5rem 0 0 1.5rem;"><i class="bi bi-search" style="font-size: 16px;"></i></span>
                    <input v-model="search" @keyup.enter="searchColor" type="text" class="form-control" aria-label="Username" aria-describedby="basic-addon1" style="border-radius: 0 1.5rem 1.5rem 0;">
                </div>
                    ...
                    <div class="modal fade" id="сreateCategoryModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
                            ...
                            <div class="modal-header text-center">
                                <h1 class="modal-title fs-5" id="exampleModalLabel">Создание цвета</h1>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
                                <div class="form-floating mt-2 mb-3">
                                    <input type="text" class="form-control" placeholder="name@example.com" v-model="newColor.name">
                                    <label for="floatingInput">Имя</label>
                                </div>
                                <div class="form-floating mb-3">
                                    <input type="text" class="form-control" placeholder="Password" v-model="newColor.slug">
                                    <label for="floatingPassword">Слаг</label>
                                </div>
                            </div>
                            <div class="modal-footer ">
                                <button @click="addColor()" type="button" class="btn btn-second w-100" data-bs-dismiss="modal">Cоздать</button>
                            </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="row flex-grow-1" id="color-list">
            <div class="col d-flex flex-column" style="flex-grow: 1;">
                ...
                <div v-if="!loading">
                    <div v-if="colors.colors.length != 0">
                        ...
                        :handleColorDeleted = "handleColorDeleted"
                        :handleColorUpdated = "handleColorUpdated"
                        :loading="loading"
                        :showErrorToast="showErrorToast"/>

                    </div>
                    <div v-else>
                        <span style="font-size: 1.3rem;">Цветов пока нет ...</span>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="col text-center">
                <nav class="px-0 py-0" aria-label="Навигация по страницам">
                    <ul class="my-2 pagination justify-content-center">
                        <li class="page-item page-item-pointer" v-if="colors.totalPages > 1 && currentPage > 1">
                        ...
                        <li class="page-item page-item-pointer" v-if="colors.totalPages > 1 && currentPage < colors.totalPages">
        ...
        <div class="toast-container position-fixed bottom-0 end-0 p-3">
            <div id="successfulCreationColorToast" class="toast text-bg-success" role="alert" aria-live="assertive" aria-atomic="true" data-bs-delay="3000">
                <div class="d-flex">
...
export default {
    ...
    computed: {
        displayedPages() {
            ...
            const endPage = Math.min(this.colors.totalPages, startPage + totalDisplayPages - 1);
            ...
            if (endPage < this.colors.totalPages) {
            pages.push('...', this.colors.totalPages);
            }
    ...
    async mounted() {
        this.currentPage = localStorage.getItem('colorCurrentPage');

        if (!this.currentPage) {
            this.currentPage = 1;
            localStorage.setItem('colorCurrentPage', this.currentPage);
        }

        this.selectedSort = localStorage.getItem('colorSelectedSort');

        if (!this.selectedSort) {
            this.selectedSort = 'none';
            localStorage.setItem('colorSelectedSort', this.selectedSort);
        }

        await this.getColors(this.currentPage, this.selectedSort);
    },
    methods: {
        async getColors(page, selectedSort) {
        ...
        async addColor() {
            const formData = {
                ...
                slug: this.newColor.slug,
            }
            console.log(formData)
            try {
                this.loading = true;
                const response = await axios.post(`/color`, formData);
                if (response.status == 200) {
                    this.showsuccessfulCreationColorToast()
                    this.newColor.name = '',
                    this.newColor.slug = ''
                }
                else {
        ...
        async changePage(page) {
            this.loading = true;
            localStorage.setItem('colorCurrentPage', page);
            this.currentPage = page
            await this.getColors(page, this.selectedSort);
            this.loading = false;
        },
        async changeSort() {
            this.loading = true;
        ...
        showsuccessfulCreationColorToast() {
            const successfulCreation = new Toast(document.getElementById('successfulCreationColorToast'))
        ...
        async handleColorDeleted() {
            await this.getColors(this.currentPage, this.selectedSort);
        },
        async handleColorUpdated() {
            await this.getColors(this.currentPage, this.selectedSort);
        }
    },
}     
