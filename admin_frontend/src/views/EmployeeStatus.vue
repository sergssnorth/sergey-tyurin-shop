<template>
    <div class="container-fluid text-center" style="height: 100%; display: flex; flex-direction: column;">
        <div class="row mb-3">
            <div class="d-flex align-items-center flex-grow-1">
                <div class="input-group align-items-center">
                    <span @click="searchEmployeeStatus" class="input-group-text" id="basic-addon1"
                        style="border-radius: 1.5rem 0 0 1.5rem;"><i class="bi bi-search"
                            style="font-size: 16px;"></i></span>
                    <input v-model="search" @keyup.enter="searchEmployeeStatus" type="text" class="form-control"
                        ...
                        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
                            <div class="modal-content">
                                <div class="modal-header text-center">
                                    <h1 class="modal-title fs-5" id="exampleModalLabel">Создание статуса работника</h1>
                                ...
                                <div class="modal-body">
                                    <div class="form-floating mt-2 mb-3">
                                        <input type="text" class="form-control" placeholder="name@example.com"
                                            v-model="newEmployeeStatus.name">
                                        <label for="floatingInput">Наименование</label>
                                    </div>
                                </div>
                                <div class="modal-footer ">
                                    <button @click="addEmployeeStatus()" type="button" class="btn btn-second w-100"
        ...
        <div class="row flex-grow-1" id="employeeStatus-list">
            <div class="col d-flex flex-column" style="flex-grow: 1;">
                ...
                <div v-if="!loading">
                    <div v-if="employeeStatuses.employeeStatuses.length != 0">
                        <ListEmployeeStatuses 
                        v-for="employeeStatus in employeeStatuses.employeeStatuses" 
                        :key="employeeStatus.id"
                        :employeeStatus="employeeStatus"

                        :setLoading="setLoading"
                        :handleEmployeeStatusDeleted="handleEmployeeStatusDeleted"
                        :handleEmployeeStatusUpdated="handleEmployeeStatusUpdated" 
                        :loading="loading"
            ...
            <div class="col text-center">
                <nav class="px-0 py-0" aria-label="Навигация по страницам">
                    <ul class="my-2 pagination justify-content-center">
                        <li class="page-item page-item-pointer" v-if="employeeStatuses.totalPages > 1 && currentPage > 1">
                        ...
                        <li class="page-item page-item-pointer"
                            v-if="employeeStatuses.totalPages > 1 && currentPage < employeeStatuses.totalPages">
        ...
        <div class="toast-container position-fixed bottom-0 end-0 p-3">
            <div id="successfulCreationEmployeeStatusToast" class="toast text-bg-success" role="alert" aria-live="assertive"
                aria-atomic="true" data-bs-delay="3000">
                <div class="d-flex">
                    <div class="toast-body">
                        Статус сотдрудника успешно создан!
                    </div>
...
import ListEmployeeStatuses from '@/components/ListEmployeeStatuses'
...
export default {
    ...
    computed: {
        displayedPages() {
            ...
            const endPage = Math.min(this.employeeStatuses.totalPages, startPage + totalDisplayPages - 1);
            ...
            if (endPage < this.employeeStatuses.totalPages) {
                pages.push('...', this.employeeStatuses.totalPages);
            }
    ...
    async mounted() {
        this.currentPage = localStorage.getItem('employeeStatusCurrentPage');

        if (!this.currentPage) {
            this.currentPage = 1;
            localStorage.setItem('employeeStatusCurrentPage', this.currentPage);
        }

        this.selectedSort = localStorage.getItem('employeeStatusSelectedSort');

        if (!this.selectedSort) {
            this.selectedSort = 'none';
            localStorage.setItem('employeeStatusSelectedSort', this.selectedSort);
        }

        await this.getEmployeeStatuses(this.currentPage, this.selectedSort);
    },
    methods: {
        async getEmployeeStatuses(page, selectedSort) {
            ...
            try {
                this.loading = true;
                const response = await axios.get(`/employee-statuses`, { params });
        ...
        async addEmployeeStatus() {
            const formData = {
                name: this.newEmployeeStatus.name,
            }
            console.log(formData)
            try {
                this.loading = true;
                const response = await axios.post(`/employee-status`, formData);
                if (response.status == 200) {
                    this.showSuccessfulCreationEmployeeStatusToast()
                    this.newEmployeeStatus.name = ''
                }
                else {
                    this.showErrorToast(response.status, response.data)
                    console.log(response);
        ...
        async changePage(page) {
            this.loading = true;
            localStorage.setItem('employeeStatusCurrentPage', page);
            this.currentPage = page
            await this.getEmployeeStatuses(page, this.selectedSort);
            this.loading = false;
        },
        async changeSort() {
            this.loading = true;
        ...
        showSuccessfulCreationEmployeeStatusToast() {
            const successfulCreation = new Toast(document.getElementById('successfulCreationEmployeeStatusToast'))
        ...
        async handleEmployeeStatusDeleted() {
            console.log("handleCategoryDeleted")
            await this.getEmployeeStatuses(this.currentPage, this.selectedSort);
        },
        async handleEmployeeStatusUpdated() {
            await this.getEmployeeStatuses(this.currentPage, this.selectedSort);
        }
    },
}
