...
# async def get_warehouse_elements(offset: int = Query(0, ge=0),
...
#     query = select(WarehouseElement, ProductInstance, Product, Size, Model).join(ProductInstance).join(Product).join(Size).join(Model)
...
#         if sort_by == "id":
#             query = query.order_by(WarehouseElement.id.desc() if order == "desc" else WarehouseElement.id.asc())
#     else:
...
#     warehouse_elements = [WarehouseElementResponseModel(
...
#         ) for warehouse_element, Product_Instance, Product, model   in warehouse_elements]
